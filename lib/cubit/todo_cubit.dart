...
import 'package:employee/screens/done_screen.dart';
import 'package:employee/screens/tasks_screen.dart';
...
class AppCubit extends Cubit<AppStates> {
  ...
  List<String> titles = ['Add new employee', 'The employees', 'Deleted employees'];
  int currentIndex = 0;
  ...
  void creatDatabase() {
    openDatabase('todo.db', // name of project
        version: 1,
      ...
      database
          .execute(
              'CREATE TABLE tasks(id INTEGER PRIMARY KEY ,title TEXT,date TEXT,time TEXT,name TEXT,status TEXT)')
          .then((value) {
        print('table created ');
      }).catchError((error) {
  ...
  Future insertToDatabase({
    ...
    try {
      await database!.transaction((txn) async {
        var value = await txn
            .rawInsert(
          'INSERT INTO tasks(title,date,time,name,status) VALUES("$title","$date", "$time","$name", "new")',
